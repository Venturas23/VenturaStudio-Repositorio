...
def search_tv_show_on_tmdb(series_title, api_token):
    """
    Busca por uma série no TMDB e retorna informações relevantes.
    """
    if not series_title:
        return None
    ...
    for item in movies_from_m3u:
        ...
        if series_name not in series_dict:
            series_dict[series_name] = {
                "name": series_name,
                "category": category, # Salva a categoria do M3U
                "tmdb_info": {},
                "episodes": {}
            }
        ...
        series_dict[series_name]["episodes"][episode_number] = {
            "full_name": full_name,
            "link": link,
            "m3u_logo": tvg_logo # Salva a logo original do M3U para o episódio
        }
